...
- [~] **TC-3.4** `fw_setenv` / `fw_printenv` round-trip from userspace — partial sign-off reverted 2026-04-24 after session-5 investigation (see learnings below). The Linux ↔ eMMC half (libubootenv round-trip) is verified on commit `14e0887`, but serial logs showed U-Boot was falling through to the NOWHERE env driver on every boot (`*** Warning - bad CRC, using default environment`). Fragment fix landed in `1b45368` — re-sign-off pending a build+reflash that verifies `Loading Environment from MMC... OK` on serial.
